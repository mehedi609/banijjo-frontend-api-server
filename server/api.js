...
router.get('/products', async (req, res) => {
  try {
    ...
    if(search_text) db_products = db_products.filter(product => product.product_name.toLowerCase().indexOf(search_text.toLowerCase()) !== -1)
    if(orderway) orderway == 'desc' ? db_products.sort((a, b) => b.product_name - a.product_name) : db_products.sort((a, b) => a.product_name - b.product_name)
    if(offset) db_products = db_products.slice(offset)
    if(limit) db_products = db_products.slice(0, limit)

    let products = [];
    db_products.length > 0 && db_products.forEach(element => {
      products.push({
...
router.get('/top_main_banners', async (req, res) => {
  try {
    const mainBanners = await query(
      `SELECT id, name, image, url FROM banner WHERE softDel=0 AND status=1`,
    );
...
router.get('/all_category_list', async (req, res) => {
  ...
  SELECT 
    category.id,category.category_name,category_order.status 
  FROM 
...
router.get('/vendors', async (req, res) => {
  try {
    const vendors = await query(`SELECT vd.id, vd.name, vd.vendor_id, vd.logo 
...
router.post('/checkInventory', async (req, res) => {
  try {
    ...
    for (const { id, quantity, color, size } of cartProducts) {
      ...
      console.log(`${id},${color.id},${size.id} --- ${netProductQuantity}`);
...
router.get('/productCombinationsFromStock/:p_id', async (req, res) => {
  ...
  try {
    const combinations = await query(`SELECT productId, colorId, sizeId, SUM(quantity) as quantity
                                FROM stock WHERE productId=${p_id} AND status = 1 AND softDel=0 
